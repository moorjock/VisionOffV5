...
//
// I have made some minor changes to fix a compilation issue and fit in with my codes 
...
// it only needs the following #includes
...
// macro-like inline functions, they may cause nameclashes with GPU code
...
{printf("ERROR: %s\n     in file %s at line %d\n",(const char*)message,__FILE__,__LINE__); throw(1);}
#else
struct NRerror {
	char *message;
	char *file;
...
void NRmatrix<T>::resize(int newn, int newm)
{
	...
	if (newn != nn || newm != mm) {
		...
		mm = newm;
//####
// the folowing line(s) fail (cause a ntdll breakpoint) in MS VisualStudio
// in debug mode for large(ish) matrix sizes (20x20)
//			v = nn>0 ? new T*[nn] : NULL;
//			nel = mm*nn;
//			if (v) v[0] = nel>0 ? new T[nel] : NULL;
//			for (i=1; i< nn; i++) v[i] = v[i-1] + mm;
//
// I replaced them with the following lines (betwen \\#### ..\\####) which work
// the problem seems to relate to the use of a template T* in a new T*[nn]
// this change means that this general NRmatrix<T> class is now fixed as a double-only class
// otherwise it will fail. I don't think this is too much of an issue at present
// as I would only do Eigen and SVD analysis in double precision.
//
//####
		v=NULL;
		if (nn > 0) {
...
//typedef NRvector<Uint> VecUint, VecUint_O, NanDo;

typedef const NRvector<Llong> VecLlong_I;
typedef NRvector<Llong> VecLlong, VecLlong_NaNg_IO;

//typedef const NRvector<Ullong> VecUllong_I;NanDo;
